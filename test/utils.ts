...
export function testIfExplicitFilePathExists(
	...
	test(`Checking for ${file}`, async () => {
		try {
			...
			if (!(await expected.exists())) {
				throw new Error(
					`Could not find file '${file}' in '${expectedFileLocation}'`,
				);
			}
			if (!(await generated.exists())) {
				throw new Error(
					`Could not find file '${file}' in '${generatedFileLocation}'`,
				);
			}
			...
			if (expectedText !== generatedText) {
				...
				throw new Error(
					`File '${file}' in '${generatedFileLocation}' does not match '${expectedFileLocation}'`,
				);
			}
...
export function testIfFileExists(
	...
	return testIfExplicitFilePathExists(
...
export function testFileDoesntExist(generationDirectory: string, file: string) {
	...
	test(`Checking that ${file} does NOT exist`, async () => {
		try {
			...
			console.error(error);
			expect().fail(
				`Found '${file}' in '${generationDirectory}' when it shouldn't exist`,
			);
		}
	});
}
