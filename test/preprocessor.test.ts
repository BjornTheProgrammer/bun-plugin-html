		...
		plugins: [
			html({
				async preprocessor(processor) {
					...
					processor.writeFile(
						path.resolve(startingDirectory, './js/third.js'),
					...
					for (const file of files) {
						if (file.extension === '.css') {
							const contents =
								await $`bun run tailwindcss -i ${file.path} --content '${startingDirectory}**/*.{html,js,ts}'`
									.quiet()
									.text();
					...
					processor.writeFile(
						path.resolve(startingDirectory, 'hello.txt'),
						'Hello World!',
					);
				},
			}),
